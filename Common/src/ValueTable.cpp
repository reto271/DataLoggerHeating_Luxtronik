...
void ValueTable::initValueTable()
{
    ...
    //  |----|-------------------------------------|----------------------------------------------------------|------------|------------|------------|-------|-------|--------|-------------------------|
        {  10, /*ID_WEB_Temperatur_TVL*/            "Vorlauftemperatur Heizkreis"                             ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
        {  11, /*ID_WEB_Temperatur_TRL*/            "Rücklauftemperatur Heizkreis"                            ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
        {  12, /*ID_WEB_Sollwert_TRL_HZ*/           "Rücklauf-Soll Heizkreis"                                 ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
    //  {  13, /*ID_WEB_Temperatur_TRL_ext*/        "Rücklauftemperatur im Trennspeicher"                     ,          10, "°C"}      ,                                                                 -> deactivate
        {  14, /*ID_WEB_Temperatur_THG*/            "Heisgastemperatur"                                       ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
        {  15, /*ID_WEB_Temperatur_TA*/             "Aussentemperatur"                                        ,          10, "°C"       ,        true,      1,     "",      11, DataTypeInfo::SIGNED   }, // -102.4 .. 102.3
        {  16, /*ID_WEB_Mitteltemperatur*/          "Durchschnittstemperatur Aussen (24 h)"                   ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::SIGNED   }, // -51.2 .. 51.1
        {  17, /*ID_WEB_Temperatur_TBW*/            "Warmwasser Ist-Temperatur"                               ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
        {  18, /*ID_WEB_Einst_BWS_akt*/             "Warmwasser Soll-Temperatur"                              ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
        {  19, /*ID_WEB_Temperatur_TWE*/            "Wärmequellen-Eintrittstemperatur"                        ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
        {  20, /*ID_WEB_Temperatur_TWA*/            "Wärmequellen-Austrittstemperatur"                        ,          10, "°C"       ,        true,      1,     "",      10, DataTypeInfo::UNSIGNED }, // 0..102.3
    ...
    //  {  55, /*ID_WEB_MA2out_Ausgang*/            "Mischkreis 2 Auf"                                        ,           1, "bool"}    ,                                                                 -> deactivate
        {  56, /*ID_WEB_Zaehler_BetrZeitVD1*/       "Betriebsstunden Verdichter 1"                            ,           1, "Sekunden" ,        true,     60,  "min",      30, DataTypeInfo::UNSIGNED }, // 0 .. 34 years
    ...
    //  {  62, /*ID_WEB_Zaehler_BetrZeitZWE3*/      "Betriebsstunden Zweiter Wärmeerzeuger 3"                 ,           1, "Sekunden"},                                                                 -> deactivate
        {  63, /*ID_WEB_Zaehler_BetrZeitWP*/        "Betriebsstunden Wärmepumpe"                              ,           1, "Sekunden" ,        true,     60,  "min",      30, DataTypeInfo::UNSIGNED }, // 0 .. 34 years
        {  64, /*ID_WEB_Zaehler_BetrZeitHz*/        "Betriebsstunden Heizung"                                 ,           1, "Sekunden" ,        true,     60,  "min",      30, DataTypeInfo::UNSIGNED }, // 0 .. 34 years
        {  65, /*ID_WEB_Zaehler_BetrZeitBW*/        "Betriebsstunden Warmwasser"                              ,           1, "Sekunden" ,        true,     60,  "min",      30, DataTypeInfo::UNSIGNED }, // 0 .. 34 years
    //  {  66, /*ID_WEB_Zaehler_BetrZeitKue*/       "Betriebsstunden Kühlung"                                 ,           1, "Sekunden"},                                                                 -> deactivate
        {  67, /*ID_WEB_Time_WPein_akt*/            "Wärmepumpe läuft seit"                                   ,           1, "Sekunden" ,        true,     60,  "min",      17, DataTypeInfo::UNSIGNED }, // 0 .. 36h
    //  {  68, /*ID_WEB_Time_ZWE1_akt*/             "Zweiter Wärmeerzeuger 1 läuft seit"                      ,           1, "Sekunden"},                                                                 -> deactivate
        ...
        {  70, /*ID_WEB_Timer_EinschVerz*/          "Netzeinschaltverzögerung"                                ,           1, "Sekunden" ,        true,     60,  "min",      17, DataTypeInfo::UNSIGNED }, // 0 .. 36h
        {  71, /*ID_WEB_Time_SSPAUS_akt*/           "Schaltspielsperre Aus"                                   ,           1, "Sekunden" ,        true,     60,  "min",      17, DataTypeInfo::UNSIGNED }, // 0 .. 36h
        {  72, /*ID_WEB_Time_SSPEIN_akt*/           "Schaltspielsperre Ein"                                   ,           1, "Sekunden" ,        true,     60,  "min",      17, DataTypeInfo::UNSIGNED }, // 0 .. 36h
        {  73, /*ID_WEB_Time_VDStd_akt*/            "Verdichter-Standzeit"                                    ,           1, "Sekunden" ,        true,     60,  "min",      21, DataTypeInfo::UNSIGNED }, // 0 .. 36h -> Not enough if on vacation. New: 21bit -> 3 weeks
        {  74, /*ID_WEB_Time_HRM_akt*/              "Heizungsregler Mehr-Zeit"                                ,           1, "Sekunden" ,       false,     60,  "min",      17, DataTypeInfo::UNSIGNED }, // 0 .. 36h
        {  75, /*ID_WEB_Time_HRW_akt*/              "Heizungsregler Weniger-Zeit"                             ,           1, "Sekunden" ,       false,     60,  "min",      21, DataTypeInfo::UNSIGNED }, // 0 .. 36h -> Not enough if on vacation. New: 21bit -> 3 weeks
        {  76, /*ID_WEB_Time_LGS_akt*/              "Thermische Desinfektion läuft seit"                      ,           1, "Sekunden" ,        true,     60,  "min",      17, DataTypeInfo::UNSIGNED }, // 0 .. 36h -> Heats up to 60 deg
    ...
    //- 150|ID_WEB_WebsrvProgrammWerteBeobarten|??|??|??
        { 151, /*ID_WEB_WMZ_Heizung*/               "Wärmemengenzähler Heizung"                               ,          10, "kWh"      ,        true,      1,     "",      28, DataTypeInfo::UNSIGNED }, // 460 times more than today
        { 152, /*ID_WEB_WMZ_Brauchwasser*/          "Wärmemengenzähler Brauchwasser"                          ,          10, "kWh"      ,        true,      1,     "",      26, DataTypeInfo::UNSIGNED }, // 382 times more than today
    //- 153|ID_WEB_WMZ_Schwimmbad|Wärmemengenzähler Schwimmbad|Wert / 10|kWh
        { 154, /*ID_WEB_WMZ_Seit*/                  "Wärmemengenzähler Gesamt"                                ,          10, "kWh"      ,        true,      1,     "",      28, DataTypeInfo::UNSIGNED }, // 350 times more than today
